...
from champmetrics.lib import topoproject, riverscapes
...
def generate_substrate_raster(topo_project_folder, out_path, di_values, dict_ocular_values, out_channel_value=4000.0):
    ...
    for tagname, tags in {"Site": ["Site", "SiteName"], "Visit": ["Visit", "VisitID"], "Year": ["Year", "FieldSeason"],
                          "Watershed": ["Watershed", "Watershed"]}.items():
    ...
    for di in di_values:
        ...
        for unitid, dict_unit in dict_units.items():
        ...
        dict_di_roughness_values[di] = pandas.DataFrame(list(dict_roughness_values.items()),
    ...
    for di, df_roughness_values in dict_di_roughness_values.items():
