...
from champmetrics.lib.tin import TIN
from champmetrics.lib.sitkaAPI import downloadUnzipTopo
from champmetrics.lib import topoproject
...
from champmetrics.lib import riverscapes
...
def export_cad_files(project_xml, out_path):
    ...
    for tagname, tags in {"Site": ["Site", "SiteName"], "Visit": ["Visit", "VisitID"], "Year": ["Year", "FieldSeason"], "Watershed": ["Watershed", "Watershed"]}.items():
...
def export_as_dxf(dict_layers, out_dxf):
    ...
    with open(out_dxf, "wt") as f:
        f.write(header)
        for name, layer in dict_layers.items():
